...
def _prompt_supabase(f_file: str, f_snippet: str) -> str:
    table   = f_file if f_file and f_file != "unknown" else "<table_name>"
    columns = f_snippet if f_snippet else "(columns unknown)"
    return (
        f"CRITICAL — Supabase Row Level Security (RLS) not enabled on table '{table}'\n"
        f"\n"
        f"What this means:\n"
        f"  GetVouch made a real, unauthenticated HTTP request to your Supabase project\n"
        f"  and successfully read rows from the '{table}' table. No login was required.\n"
        f"  Columns visible: {columns}\n"
        f"\n"
        f"  Row Level Security (RLS) is a Supabase/PostgreSQL feature that makes every\n"
        f"  table private by default. Without it, anyone who finds your anon key (which\n"
        f"  is embedded in every Lovable/Bolt/Replit app's JavaScript bundle) can query\n"
        f"  your entire database directly from their browser or a curl command.\n"
        f"\n"
        f"How to fix — Supabase Dashboard (no code needed):\n"
        f"\n"
        f"  Step 1 — Go to: https://supabase.com/dashboard → your project\n"
        f"           → Authentication → Policies\n"
        f"\n"
        f"  Step 2 — Find the '{table}' table. Click 'Enable RLS'.\n"
        f"           (This immediately blocks all unauthenticated reads — do this NOW.)\n"
        f"\n"
        f"  Step 3 — Add a read policy so your logged-in users can still access their data:\n"
        f"           Click 'New Policy' → 'For full customization' → paste:\n"
        f"\n"
        f"    CREATE POLICY \"Users read own rows\"\n"
        f"      ON public.{table}\n"
        f"      FOR SELECT\n"
        f"      USING (auth.uid() = user_id);\n"
        f"\n"
        f"           If the table has a different owner column (e.g. owner_id, created_by),\n"
        f"           replace user_id with the correct column name.\n"
        f"\n"
        f"  Step 4 — Check ALL other tables in your project:\n"
        f"           In Supabase SQL Editor run:\n"
        f"    SELECT tablename, rowsecurity\n"
        f"    FROM pg_tables\n"
        f"    WHERE schemaname = 'public'\n"
        f"    ORDER BY tablename;\n"
        f"           Every row where rowsecurity = false is publicly readable.\n"
        f"           Repeat Steps 2-3 for each one.\n"
        f"\n"
        f"  Step 5 — Rotate your anon key:\n"
        f"           GetVouch found your anon key in the JavaScript bundle. While the\n"
        f"           anon key is designed to be public, with RLS disabled it acts as a\n"
        f"           full read (and sometimes write) credential. After enabling RLS,\n"
        f"           go to: Project Settings → API → Regenerate anon key. Then update\n"
        f"           your environment variables / Lovable project settings.\n"
        f"\n"
        f"Verify:\n"
        f"  Run this curl command — it should return an empty array [] or a 401 error,\n"
        f"  NOT your data:\n"
        f"    curl -H 'apikey: <your-anon-key>' \\\n"
        f"         -H 'Authorization: Bearer <your-anon-key>' \\\n"
        f"         'https://<project-id>.supabase.co/rest/v1/{table}?select=*&limit=1'\n"
        f"\n"
        f"  Re-scan with GetVouch — the Supabase RLS finding should be gone."
    )
